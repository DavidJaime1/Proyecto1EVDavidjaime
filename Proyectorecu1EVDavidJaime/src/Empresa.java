...
public class Empresa {
    ...
    public String getSedes() {
        StringBuilder sedes = new StringBuilder();

        for (String sede : grupo.keySet()) {
            sedes.append(sede).append("\n");
        }

        return sedes.toString();
    }

    public void buscarCochesPorMarca(String marca) {
        for (String sede : grupo.keySet()) {
            Concesionario concesionario = grupo.get(sede);
            List<Coche> cochesEncontrados = concesionario.buscarCochesPorMarca(marca);
            System.out.println("En el concesionario " + sede + " se encontraron " + cochesEncontrados.size() + " coches.");
            cochesEncontrados.forEach(System.out::println);
        }
    }

    public void buscarCochesPorModelo(String modelo) {
        for (String sede : grupo.keySet()) {
            Concesionario concesionario = grupo.get(sede);
            List<Coche> cochesEncontrados = concesionario.buscarCochesPorModelo(modelo);
            System.out.println("En el concesionario " + sede + " se encontraron " + cochesEncontrados.size() + " coches.");
            cochesEncontrados.forEach(System.out::println);
        }
    }

    public void mostrarListadoPorConcesionario(String nombreConcesionario) {
        Concesionario concesionario = getConcesionario(nombreConcesionario);
        if (concesionario == null) {
            System.out.println("El concesionario no existe.");
            return;
        }
        System.out.println("Listado de coches en el concesionario " + nombreConcesionario + ":");
        concesionario.listarCoches().forEach(System.out::println);
    }
